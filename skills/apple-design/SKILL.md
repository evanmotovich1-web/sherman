...
name: apple-design
category: design
summary: design interfaces with Apple's fluid, physical principles
description: Apply Apple's fluid, physical interface principles to product design and implementation. Use when designing or reviewing an interaction-heavy interface.
---
...
- **Respond on pointer-down, not on release.** Highlight a button the instant it's pressed. Waiting for `click` or touch-up to show feedback feels dead.
