...
Adapted from ScrapeCreators/social-media-research-skills (github.com/ScrapeCreators/social-media-research-skills, per-skill MIT), audited before adoption.

Sherman boundary: the SCRAPECREATORS_API_KEY is a machine secret — never print, log, or store its value, and never write it into the vault. If the key is not set, say so and hand over the one-line checklist (get a key at scrapecreators.com, export SCRAPECREATORS_API_KEY) or ask /0-1 to wire it. Public social data only; nothing patient-adjacent, ever.
