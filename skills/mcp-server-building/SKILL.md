...
category: agent
summary: design and harden mcp servers with precise tool contracts
...
Adapted from awesome-ai-agent-skills (github.com/seb1n/awesome-ai-agent-skills, MIT), audited before adoption.

Sherman caution: this skill pins claims to a specific MCP protocol revision (2026-07-28) that the adoption audit could not verify. Before relying on any revision-specific detail — session headers, discovery methods, manifest fields — verify it against the live MCP specification; the trust-boundary and tool-contract methodology stands regardless.
