...
category: design
summary: find valuable missing motion in a product interface
description: Inspect a codebase or interface for places where motion would improve clarity, continuity, or feedback. Use when asked where animation should be added.
---
...
Useful sweeps: search for conditional renders with no transition (`{isOpen &&`, `display: none` toggles), `onClick` handlers on elements with no `:active` or transition styles, `details` and accordion markup, drag handlers, `.map(` renders of entering lists, empty-state and success components.
