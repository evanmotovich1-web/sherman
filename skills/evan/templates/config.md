...
Template for `.evan/config.md` - project-specific configuration and integrations.

**Purpose:** Store project settings and optional integration flags. Allows EVAN to adapt behavior based on available tools.
...
Optional integrations that enhance EVAN functionality.
...
**When enabled:**
- `/evan quality-gate` runs scans and updates CONCERNS.md
...
**When enabled:**
- `/evan plan` suggests running `/evan audit` before APPLY
- `/evan audit` performs enterprise-grade architectural review
...
Optional user preferences for EVAN behavior.
...
- User preferences for EVAN behavior
...
- During `/evan init` if user enables integrations
- Manually when adding integrations later
- Not required for basic EVAN usage
