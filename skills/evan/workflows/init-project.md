<purpose>
Initialize EVAN structure in a new project. Creates .evan/ directory with PROJECT.md, ROADMAP.md, STATE.md, and phases/ directory. Gathers project context through a type-adapted requirements walkthrough before populating files — instead of leaving skeleton placeholders for planning to fill.
</purpose>

<when_to_use>
- Starting EVAN in a project that doesn't have .evan/ directory
...
1. Check if .evan/ directory exists:
   ```bash
   ls .evan/ 2>/dev/null
   ```
2. If exists:
   - "EVAN already initialized in this project."
   - Route to `/evan resume` or `/evan progress`
   - Exit this workflow
...
2. **If found:**
   - Display:
     ...
     This file contains project requirements that can populate your EVAN setup
...
**Determine what kind of project this is — instead of assuming everything is a software application, because EVAN manages campaigns, workflows, and other non-code projects equally well.**
...
phased breakdown, design decisions, data model design — EVAN has a companion
tool called SEED that specializes in this.

SEED produces a PLANNING.md that EVAN can consume directly.

[1] Install SEED and start there (recommended for complex projects)
[2] Continue with EVAN init using what we've gathered
```

Wait for user response.

**If "1" or "install":**
1. Run: `npx evan framework add-skill seed` (or applicable install command)
...
Create `.evan/PROJECT.md` with index-compatible frontmatter:
...
**Fallback:** If any section has no data from the walkthrough (user skipped or answered minimally), use a single-line placeholder: `- To be defined during /evan plan` — instead of multi-line placeholder blocks, because minimal placeholders signal "not yet explored" without cluttering the document.
</step>

<step name="create_roadmap_md">
Create `.evan/ROADMAP.md` with index-compatible frontmatter:
...
Phases will be defined during `/evan plan`.
...
Create `.evan/STATE.md` with index-compatible frontmatter:
...
See: .evan/PROJECT.md (updated [timestamp])
...
Next action: Run /evan plan to define phases and first plan
Resume file: .evan/PROJECT.md
...
(Don't create config.md - user can add later via /evan config)
</step>
...
[1] Yes, configure now
[2] Skip for now (add later via /evan flows)
```
...
(User can add later via /evan flows)
</step>
...
Created:
  .evan/PROJECT.md    ✓  (requirements populated from walkthrough)
  .evan/ROADMAP.md    ✓
  .evan/STATE.md      ✓
  .evan/config.md     ✓  (if integrations_enabled: list enabled integrations)
  .evan/SPECIAL-FLOWS.md  ✓  (if specialized_flows_enabled: "[N] skills configured")
  .evan/phases/       ✓
...
- Example: `.evan/config.md ✓ (SonarQube, Enterprise Plan Audit enabled)`
...
<output>
- `.evan/` directory structure
- `.evan/PROJECT.md` (populated from walkthrough or PLANNING.md import)
- `.evan/ROADMAP.md` (skeleton for planning)
- `.evan/STATE.md` (initialized state)
- `.evan/config.md` (if integrations enabled)
- `.evan/SPECIAL-FLOWS.md` (if specialized flows enabled)
- `.evan/phases/` (empty directory)
- Clear routing to `/evan plan`
</output>
...
- Use single-line placeholder: "- To be defined during /evan plan"
