...
.evan/ROADMAP.md
.evan/PROJECT.md
.evan/phases/{prior-phase}/{plan}-SUMMARY.md (if exists and relevant)
</required_reading>
...
4. Display reminder if required skills found:
   ...
   These must be loaded before /evan apply will proceed.
   ════════════════════════════════════════
   ```
...
1. Create phase directory: `.evan/phases/{NN}-{phase-name}/`
...
1. Check if `.evan/config.md` exists
...
1. **Update STATE.md** with exact content:
   ...
   Next action: Review and approve plan, then run /evan apply [plan-path]
...
3. **Report with quick continuation prompt:**
   ...
   **If `audit_enabled` is true:**
   ...
   ```
   Accept quick inputs: "1", "audit" → run `/evan audit [plan-path]`
   Accept quick inputs: "2", "skip", "apply" → run `/evan apply [plan-path]`
   ...
   **If `audit_enabled` is false (default):**
   ...
   ```
   Accept quick inputs: "1", "approved", "yes", "go" → run `/evan apply [plan-path]`
</step>
...
<output>
PLAN.md at `.evan/phases/{NN}-{phase-name}/{NN}-{plan}-PLAN.md`

Example: `.evan/phases/04-workflows-layer/04-01-PLAN.md`
</output>
