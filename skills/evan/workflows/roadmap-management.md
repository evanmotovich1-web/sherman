...
<references>
templates/ROADMAP.md (phase section format)
</references>
...
- Error: "No active milestone. Run /evan milestone first."
- Exit workflow
</step>
...
```bash
mkdir -p .evan/phases/{NN}-{phase-slug}
```
...
- Delete .evan/phases/{slug}/ (if empty)
...
```bash
rmdir .evan/phases/{NN}-{slug} 2>/dev/null
```

**If directory not empty:**
- Warn: "Directory not empty — preserved at .evan/phases/{slug}/"
...
```bash
mv .evan/phases/08-name .evan/phases/07-name
mv .evan/phases/09-name .evan/phases/08-name
```
...
<error_handling>
**No active milestone:**
- Route to /evan milestone or /evan init
