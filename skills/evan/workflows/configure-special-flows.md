<purpose>
Configure specialized skill integrations for a EVAN project. Discovers available skills, prompts user to select applicable ones, maps skills to work types, and generates .evan/SPECIAL-FLOWS.md.
</purpose>

<when_to_use>
- During init when user has specialized skills/commands
- Ad-hoc via `/evan flows` command
...
<required_reading>
.evan/PROJECT.md (project context)
.evan/SPECIAL-FLOWS.md (if exists, for amendment)
</required_reading>

<references>
references/specialized-workflow-integration.md
templates/SPECIAL-FLOWS.md
</references>

<process>

<step name="check_existing" priority="first">
1. Check if .evan/SPECIAL-FLOWS.md exists:
   ```bash
   ls .evan/SPECIAL-FLOWS.md 2>/dev/null
   ```
2. If exists:
   ```
...
1. Create .evan/SPECIAL-FLOWS.md using template:
...
3. Display confirmation:
   ...
   Run /evan flows audit to check current phase.
   ────────────────────────────────────────
   ```
</step>
...
1. Read .evan/SPECIAL-FLOWS.md
2. Read .evan/STATE.md for current phase
...
<output>
- `.evan/SPECIAL-FLOWS.md` created or updated
- `.evan/PROJECT.md` updated with quick reference
...
**No .evan/ directory:**
```
EVAN not initialized in this project.
Run /evan init first, or create .evan/ manually.
```

**User provides invalid skill name:**
- Warn but proceed (skill may exist elsewhere)
- Note: "Skill not found in skills/ but adding anyway"
</error_handling>
