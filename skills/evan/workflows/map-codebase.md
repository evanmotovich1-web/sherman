<purpose>
Orchestrate parallel Explore agents to analyze codebase and produce structured documents in .evan/codebase/
...
Check if .evan/codebase/ already exists:

```bash
ls -la .evan/codebase/ 2>/dev/null
```

**If exists:**

```
.evan/codebase/ already exists with these documents:
[List files found]
...
If "Refresh": Delete .evan/codebase/, continue to create_structure
...
Create .evan/codebase/ directory:

```bash
mkdir -p .evan/codebase
```
...
4. **Write to .evan/codebase/{NAME}.md** (uppercase filename)
...
If .evan/STATE.md exists, note that codebase has been mapped.

Add to Accumulated Context section:
```
### Codebase Mapped
Date: [current date]
Documents: .evan/codebase/ (7 files)
```
...
**Initialize EVAN project** — use codebase context for planning
...
**Also available:**
- Re-run mapping: `/evan map-codebase`
- Review specific file: `cat .evan/codebase/STACK.md`
...
<success_criteria>
- .evan/codebase/ directory created
