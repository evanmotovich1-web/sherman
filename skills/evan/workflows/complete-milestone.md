...
After completion, project is ready for /evan discuss-milestone or /evan milestone.
</loop_context>
...
.evan/PROJECT.md
.evan/ROADMAP.md
.evan/phases/{milestone-phases}/*-SUMMARY.md
</required_reading>

<references>
templates/MILESTONES.md (entry format)
templates/milestone-archive.md (archive format)
</references>
...
Create or update `.evan/MILESTONES.md`:
...
1. Create directory:
   ```bash
   mkdir -p .evan/milestones
   ```

2. Create archive file `.evan/milestones/{version}-ROADMAP.md`:
...
2. **Add Next Milestone placeholder:**
   ```markdown
   ## Next Milestone
   Run /evan discuss-milestone or /evan milestone to define.
   ```
...
4. **Session Continuity:**
   ...
   Next action: /evan discuss-milestone or /evan milestone
   Resume file: .evan/MILESTONES.md
   ```
</step>
...
|----------|-------|---------|
| `.evan/PROJECT.md` | Current State table → Version | `0.3.0` |
| `.evan/ROADMAP.md` | Version Overview table | `v0.3` |
| `.evan/STATE.md` | Version field | `v0.3.0` |
| `.evan/config.md` | version field (if exists) | `0.3.0` |
...
grep -A5 "Current State" .evan/PROJECT.md | grep "Version"

# ROADMAP.md - check Version Overview
grep -A10 "Version Overview" .evan/ROADMAP.md

# STATE.md - Version field
grep "^Version:" .evan/STATE.md

# config.md (if exists)
[ -f .evan/config.md ] && grep "version:" .evan/config.md
...
```bash
git add .evan/PROJECT.md .evan/ROADMAP.md .evan/STATE.md
[ -f .evan/config.md ] && git add .evan/config.md
...
Created:
  .evan/MILESTONES.md entry    ✓
  .evan/milestones/{version}-ROADMAP.md    ✓
  git tag: {version}    ✓
...
────────────────────────────────────────
▶ NEXT: /evan discuss-milestone
  Define the scope for the next milestone
────────────────────────────────────────

Or /evan milestone to create milestone directly.
```
</step>
...
- .evan/milestones/{version}-ROADMAP.md archive created
...
- [ ] Archive file created in .evan/milestones/
