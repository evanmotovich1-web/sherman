<purpose>
Resume EVAN work after a session break. Reads STATE.md to restore context, determines current loop position, and routes to exactly ONE next action. Includes handoff lifecycle management.
</purpose>

<when_to_use>
- Starting a new session on an existing EVAN project
- Context was cleared (new conversation)
- Handoff from another session
- User asks to "continue" or "resume" EVAN work
</when_to_use>
...
<step name="verify_evan_exists" priority="first">
1. Check for .evan/ directory:
   ```bash
   ls .evan/STATE.md 2>/dev/null
   ```
2. If not found:
   - "No EVAN project found. Run /evan init first."
   - Exit workflow
...
1. Search for handoff files:
   ```bash
   ls -t .evan/HANDOFF*.md 2>/dev/null | head -5
   ```
...
3. If no argument but handoffs found:
   ...
   - Note: `.evan/HANDOFF-{context}.md` is standard pattern
...
|------------|-------------------|
| PLAN ○ (no plan yet) | `/evan plan` |
| PLAN ✓, APPLY ○ (plan awaiting approval) | `/evan apply [plan-path]` |
| PLAN ✓, APPLY ✓, UNIFY ○ (executed, not reconciled) | `/evan unify [plan-path]` |
| All ✓ (loop complete) | `/evan plan` (next phase) |
...
Show exactly ONE suggested action with the standard EVAN routing format.
</step>
...
1. **Archive handoff** (if one was consumed):
   ```bash
   mkdir -p .evan/handoffs/archive
   mv .evan/HANDOFF-{context}.md .evan/handoffs/archive/
   ```
...
2. **Clean orphaned handoffs:**
   ...
   find .evan -maxdepth 1 -name "HANDOFF*.md" -mtime +7 -type f
   ```
...
- Suggest: `/evan init` to reinitialize (destructive) or manual repair
