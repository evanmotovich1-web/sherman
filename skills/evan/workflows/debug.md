...
```bash
ls .evan/debug/*.md 2>/dev/null | grep -v resolved
```
...
```bash
mkdir -p .evan/debug/resolved
mv .evan/debug/[slug].md .evan/debug/resolved/
```
...
Debug session: .evan/debug/resolved/[slug].md"
```
...
Session archived: .evan/debug/resolved/[slug].md
```
</step>
