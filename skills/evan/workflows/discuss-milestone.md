...
- Before /evan milestone when planning is desired
</when_to_use>

<loop_context>
N/A - This is a pre-planning workflow, not a loop phase.
After discussion, routes to /evan milestone (create-milestone).
</loop_context>

<required_reading>
.evan/STATE.md
.evan/PROJECT.md (current requirements and progress)
.evan/MILESTONES.md (previous milestone accomplishments, if exists)
</required_reading>

<references>
templates/milestone-context.md (handoff format)
</references>
...
Create `.evan/MILESTONE-CONTEXT.md`:

```markdown
# Milestone Context

**Generated:** {date}
**Status:** Ready for /evan milestone
...
*This file is temporary. It will be deleted after /evan milestone creates the milestone.*
```

Display:
```
Context saved to .evan/MILESTONE-CONTEXT.md
...
**Accept:** "yes", "go", "create" → run `/evan milestone`
</step>

</process>

<output>
- .evan/MILESTONE-CONTEXT.md created (handoff file)
- Features, scope, and phases articulated
- Ready for /evan milestone to consume
</output>
...
- [ ] Clear handoff to /evan milestone
</success_criteria>
...
- Route directly to /evan milestone
