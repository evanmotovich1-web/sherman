...
- When `/evan audit` is explicitly invoked by the user
...
2. If no plan found:
   - Error: "No plan found to audit. Run /evan plan first."
   - Exit workflow
...
4. Read config.md and check for `enterprise_plan_audit: enabled: true`
   ...
   - If not enabled or config missing: display warning but allow manual invocation:
     ...
     Running audit manually. To auto-suggest after /evan plan, add to config.md:
...
---
*Audit performed by EVAN Enterprise Audit Workflow*
...
**Accept quick inputs:**
- "1", "approved", "yes", "go" → run `/evan apply [plan-path]`
...
- Ask user to confirm plan location or run /evan plan first
