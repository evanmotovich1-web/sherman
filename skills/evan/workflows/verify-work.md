...
```bash
find .evan/phases -name "*SUMMARY.md" -type f -exec ls -lt {} + | head -5
```
...
1. Create `.evan/phases/XX-name/{phase}-{plan}-UAT.md` if doesn't exist
...
**Note:** Issues go to phase-scoped UAT file, NOT global `.evan/ISSUES.md`.
</step>
...
- Route to `/evan plan` for affected phase with updated intent
...
- Route to `/evan plan-fix` with spec-level scope
...
- Route to `/evan plan-fix` with code-level scope (existing behavior)
