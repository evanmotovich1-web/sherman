...
1. Check for global issues:
   ```bash
   ls .evan/ISSUES.md 2>/dev/null
   ```

2. Check for phase-scoped UAT files:
   ```bash
   find .evan/phases -name "*-UAT.md" -type f 2>/dev/null
   ```
...
3. If no issue files found:
   ...
   - Deferred enhancements during development → .evan/ISSUES.md
   - UAT testing findings → .evan/phases/XX-name/{plan}-UAT.md
...
- Suggest command: `/evan plan 10.3 Address urgent issues [list]`
