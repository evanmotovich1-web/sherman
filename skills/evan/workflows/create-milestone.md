...
- Triggered after /evan discuss-milestone (reads context)
...
.evan/STATE.md
.evan/ROADMAP.md
.evan/MILESTONE-CONTEXT.md (if exists)
</required_reading>

<references>
templates/ROADMAP.md (milestone section format)
templates/milestone-context.md (handoff structure)
</references>
...
1. Check for MILESTONE-CONTEXT.md:
   ```bash
   cat .evan/MILESTONE-CONTEXT.md 2>/dev/null
   ```
...
3. **Add phase details section:**
   ...
   Focus: {description}
   Plans: TBD (defined during /evan plan)
   ```
...
```bash
mkdir -p .evan/phases/{NN}-{name-slug}
```
...
4. **Session Continuity:**
   ...
   Next action: /evan plan for Phase {first_phase_number}
   Resume file: .evan/ROADMAP.md
   ```
</step>
...
Created:
  .evan/phases/{phase-1-slug}/     ✓
  .evan/phases/{phase-2-slug}/     ✓
  .evan/phases/{phase-N-slug}/     ✓
...
- Phase directories created in .evan/phases/
...
- Or route to /evan init if project not initialized
</error_handling>
