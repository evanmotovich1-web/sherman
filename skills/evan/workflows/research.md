...
<references>
templates/RESEARCH.md (output format)
</references>
...
Usage: /evan research <topic> [--codebase | --web]
Examples:
  /evan research "nextjs app router patterns"
  /evan research "auth patterns in this codebase" --codebase
  /evan research "prisma vs drizzle comparison" --web
```
Exit workflow.
...
Create `.evan/research/{topic-slug}.md`:

1. Create research directory if needed:
   ```bash
   mkdir -p .evan/research
   ```
...
Topic: {topic}
Agent: {agent_type}
Output: .evan/research/{topic-slug}.md
...
<output>
- .evan/research/{topic-slug}.md created with findings
