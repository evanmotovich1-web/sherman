...
- Before /evan plan when discussion is desired
- Phase scope is unclear or has multiple possible approaches
</when_to_use>

<loop_context>
N/A - This is a pre-planning workflow, not a loop phase.
After discussion, routes to /evan plan (plan-phase).
</loop_context>

<required_reading>
.evan/STATE.md
.evan/PROJECT.md (current requirements and progress)
.evan/ROADMAP.md (phase scope and goals)
</required_reading>

<references>
templates/CONTEXT.md (handoff format)
</references>
...
Usage: /evan discuss <phase-number>
Example: /evan discuss 10
```
Exit workflow.
...
Create `.evan/phases/{NN}-{name}/CONTEXT.md`:

Use CONTEXT.md template format.

Display:
```
Context saved to .evan/phases/{NN}-{name}/CONTEXT.md
...
**Accept:** "yes", "go", "plan" → run `/evan plan`
</step>

</process>

<output>
- .evan/phases/{NN}-{name}/CONTEXT.md created (handoff file)
- Goals and approach articulated
- Ready for /evan plan to consume
</output>
...
DO: Focus on user input. Use /evan assumptions for Claude's analysis.
</anti_patterns>
