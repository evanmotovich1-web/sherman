...
<required_reading>
.evan/STATE.md
.evan/phases/{phase}/{plan}-PLAN.md
</required_reading>

<references>
references/loop-phases.md
templates/SUMMARY.md
workflows/transition-phase.md (loaded when last plan in phase)
</references>
...
Create SUMMARY.md at `.evan/phases/{phase}/{plan}-SUMMARY.md`
...
**Accept:** "1", "yes", "continue" → run `/evan plan` for next plan in same phase
</step>

<step name="execute_transition" priority="required" gate="blocking">
**If last plan in phase — TRANSITION IS MANDATORY:**

⚠️ **NEVER skip this step. This is what makes EVAN a system, not random loops.**
...
**Anti-pattern:** Closing UNIFY and immediately offering `/evan plan` for next phase WITHOUT running transition. This breaks system cohesion and skips git commits.
</step>

</process>

<output>
- SUMMARY.md at `.evan/phases/{phase}/{plan}-SUMMARY.md`
- Updated STATE.md
