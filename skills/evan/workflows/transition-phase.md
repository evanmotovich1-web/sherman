...
.evan/PROJECT.md
.evan/ROADMAP.md
.evan/phases/{current-phase}/*-SUMMARY.md
</required_reading>
...
1. Check for stale handoffs in phase directory:
   ```bash
   ls .evan/phases/{current-phase}/HANDOFF*.md 2>/dev/null
   ```
2. If found, delete them — phase is complete, handoffs are stale
3. Note: Active handoffs at `.evan/` root are preserved
</step>

<step name="evolve_project">
**Read phase summaries:**
```bash
cat .evan/phases/{current-phase}/*-SUMMARY.md
```
...
Next action: /evan plan for Phase [N+1]
...
**3. Stage phase files:**
```bash
git add .evan/phases/{phase}/ .evan/STATE.md .evan/PROJECT.md .evan/ROADMAP.md
...
State consistency is foundational to EVAN. If STATE.md, PROJECT.md, or ROADMAP.md are misaligned, all downstream work breaks — resume fails, progress tracking is wrong, context is lost.
...
**Accept:** "1", "yes", "continue" → run `/evan plan` for Phase N+1
