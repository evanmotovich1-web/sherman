...
Canonical catalog of all EVAN extension points. Plugin authors building EVAN integrations reference this document to understand where their `@` references will fire, what context is available, and what use cases each point serves.
...
- `.evan/phases/{NN}-{name}/{NN}-{plan}-PLAN.md` — the just-created plan
- `.evan/STATE.md` — updated with plan reference
- `.evan/PROJECT.md` — project requirements and constraints
- `.evan/ROADMAP.md` — phase structure and progress
...
- `.evan/STATE.md` — about to be updated with APPLY complete
...
**Available context at this point:**
- `.evan/phases/{NN}-{name}/{NN}-{plan}-SUMMARY.md` — what was actually built
- `.evan/STATE.md` — fully updated with loop closure
- `.evan/ROADMAP.md` — phase progress updated
- All modified source files — in their final state

**Example use cases:**
- **Content-pipeline plugin:** Read SUMMARY.md, extract build metadata (what was built, why, teachable insights), write structured build log to `.evan/build-logs/` for blog/course content
...
- `.evan/phases/{NN}-{name}/uat-{plan}.md` — logged issues (if any)
...
- `.evan/PROJECT.md` — evolved with phase learnings
- `.evan/STATE.md` — pointing to next phase
- `.evan/ROADMAP.md` — phase marked complete with timestamp
...
```json
{
  "extensions": [
    {
      "target": "{evan_framework}/workflows/unify-phase.md",
      "section": "extensions",
...
5. This document is the canonical list — if an extension point isn't listed here, it doesn't exist in EVAN
