...
When a checkpoint:human-verify receives failure reports OR when `/evan verify` finds issues, classify the root cause BEFORE attempting fixes — instead of patching immediately, because wrong-layer fixes produce fragile patches that break again.
...
- **After APPLY:** /evan verify UAT failures (see verify-work.md)
