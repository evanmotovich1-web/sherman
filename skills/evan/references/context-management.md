...
EVAN uses context brackets to adapt behavior based on remaining capacity:
...
<context>
.evan/STATE.md
models/user.ts  (the specific file being modified)
</context>
...
.evan/ROADMAP.md
.evan/STATE.md
.evan/phases/01-foundation/01-01-SUMMARY.md
.evan/phases/01-foundation/01-02-SUMMARY.md
models/user.ts
models/product.ts
api/routes.ts
</context>
...
.evan/phases/01-foundation/01-01-PLAN.md
.evan/phases/01-foundation/01-01-SUMMARY.md
```
...
When context is exhausted or work spans sessions, EVAN provides explicit handoff support.
...
Resume file: .evan/phases/03-references-layer/03-01-PLAN.md
Resume context:
...
### HANDOFF.md Document
For zero-context sessions or complex multi-session work. Created by `/evan pause`:
...
| Command | Purpose |
|---------|---------|
| `/evan pause` | Create HANDOFF, update STATE, prepare for break |
| `/evan resume` | Restore context from HANDOFF/STATE, suggest next action |
| `/evan progress` | Mid-session check, suggests ONE next action |
